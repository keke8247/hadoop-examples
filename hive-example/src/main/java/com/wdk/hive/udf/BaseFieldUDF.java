 ...
 * @Description
 *  自定义UDF 处理基本字段
 *  继承UDF 类 实现 evaluate方法
 * @Author wangdk, wangdk@erongdu.com
 * @CreatTime 2020/3/24 16:10
 * @Since version 1.0.0
 */
public class BaseFieldUDF extends UDF{
    ...
    /**
     * @Description:
     * @Param line 使用该自定义函数时传入的查询字段
     *          jsonKeysString 字段key
     * @return
    */
    public String evaluate(String line,String jsonKeysString){
        StringBuilder stringBuilder = new StringBuilder();  //拼接返回结果

        //获取数据 根据| 分割  前面是 时间戳 后面是json字串
        ...
        try {
            JSONObject jsonContent = new JSONObject(logContents[1]);
            JSONObject cmJosn = jsonContent.getJSONObject("cm");
            for(String key : keys){
                if(cmJosn.has(key)){
                   stringBuilder.append(cmJosn.get(key)).append("\t");
                }else {
                    stringBuilder.append("\t");
                }
            }
            stringBuilder.append(jsonContent.get("et")).append("\t");
